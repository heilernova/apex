...
export class PostgreSQLQueries {
  ...
  public insert<T extends QueryResultRow = any>(options: { table: string; data: Record<string, any>; returning?: string[] | string, arrayColumns?: string[] }): Promise<QueryResult<T>> {
    const { table, data, returning, arrayColumns } = options;
    const columns: string[] = [];
    const values: unknown[] = [];
    Object.entries(data).forEach(entry => {
      if (entry[1] === undefined) return;
      columns.push(this.parseColumnName(entry[0]));
      if (arrayColumns?.includes(entry[0]) && Array.isArray(entry[1])) {
        values.push(JSON.stringify(entry[1]));
      } else {
        values.push(entry[1]);
      }
    });
  ...
  public update<T extends QueryResultRow = any>(options: { table: string; data: Record<string, any>; condition: string; conditionParams?: unknown[]; returning?: string[] | string, arrayColumns?: string[] }): Promise<QueryResult<T>> {
    const { table, data, condition, conditionParams = [], returning, arrayColumns } = options;
    ...
    const setClause = Object.entries(data).filter((entry) => entry[1] !== undefined).map((entry, index) => {
      if (arrayColumns?.includes(entry[0]) && Array.isArray(entry[1])) {
        values.push(JSON.stringify(entry[1]));
      } else {
        values.push(entry[1]);
      }
      return `${this.parseColumnName(entry[0])} = $${index + 1 + conditionParams.length}`;
    }).join(', ');
